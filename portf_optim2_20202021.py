...
Original file is located at
    https://colab.research.google.com/drive/1229DT9f3RkvDiaVbtLUnYrgs47elTmNV
"""
...
def strat_lever_equal_risk_contr(x_init, cash_init, mu, Q, cur_prices):
    ...
    cl = [1]        # lower bounds on constraints
    cu = [1]        # upper bounds on constraints
    w0 = (x_init*cur_prices)/portfolio_value # Current weight
    # Define IPOPT problem
    nlp = ipopt.Problem(n=len(w0), m=len(cl), problem_obj=erc(), lb=lb, ub=ub, cl=cl, cu=cu)
    ...
    for strategy  in range(N_strat):
      ...
      ###################### Insert your code here ############################
        if cash[strategy][period-1]<0 :
            ...
            if strategy ==5:                                                      
              #Interest cost related to LERC needs to be considered
                cash[strategy, period-1] = portfolio_value - np.dot(cur_prices,x[strategy][period-1]) - new_tran_fee - init_value*r_rf/6
            else:
...
plt.title('Figure 2: Maximum Drawdowns of Portfolios for Seven Strategies')
plt.xlabel('Period')
...
plt.title('Figure 3: Dynamic Change with robust mean variance Strategy')
plt.xlabel('Periods')
...
df_minvar.plot(figsize=(16,12),cmap="tab20")
plt.title('Figure 4: Dynamic Change with Minimum Variance Portfolio Strategy')
plt.xlabel('Periods')
plt.ylabel('Weights of Portfolio')
plt.legend(loc='upper right', ncol = 5)
...
plt.title('Figure 5: Dynamic Change with Maximum Sharpe Ratio Portfolio Strategy')
plt.xlabel('Periods')
plt.ylabel('Weights of Portfolio')
plt.legend(loc='upper right', ncol = 5)
